...
data class PulseOximeterContinuousMeasurement(
    ...
    companion object {
        fun fromBytes(value: ByteArray): PulseOximeterContinuousMeasurement {
            ...
            val measurementStatus = 0/*if (measurementStatusPresent) parser.getIntValue(FORMAT_UINT16) else null*/
            val sensorStatus = 0/*if (sensorStatusPresent) parser.getIntValue(FORMAT_UINT16) else null*/
            if (sensorStatusPresent) parser.getIntValue(FORMAT_UINT8) // Reserved byte
            val pulseAmplitudeIndex =0.0.toFloat() /* if (pulseAmplitudeIndexPresent) parser.getFloatValue(FORMAT_SFLOAT) else null*/
