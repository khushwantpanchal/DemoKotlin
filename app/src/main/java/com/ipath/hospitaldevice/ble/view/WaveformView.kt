package com.ipath.hospitaldevice.ble.view
...
class WaveformView @JvmOverloads constructor(
    ...
    public val mSurfaceHolder: SurfaceHolder
...
//        mHeight = (int) (MeasureSpec.getSize(heightMeasureSpec)*0.95);
    ...
    fun addAmp(amp: Int) {
        ...
        if (mDataBufferIndex >= mBufferSize) {
            ...
            mCanvas = mSurfaceHolder.lockCanvas(
                Rect(
                    mLastPoint!!.x, 0,
                    (xRight + pointStep * 2).toInt(), (mHeight + mLineWidth).toInt()
                )
            )
    ...
    fun reset() {
        ...
        mLastPoint = Point(
            0,
            (mHeight - mHeight / mMaxValue.toFloat() * 128).toInt()
        )
    ...
    companion object {
        private const val TAG = ">>>WAVEFORM VIEW<<<"
    }

    init {
        val metrics = context.resources.displayMetrics
        val arr =
            context.theme.obtainStyledAttributes(attrs, R.styleable.WaveformView, defStyleAttr, 0)
